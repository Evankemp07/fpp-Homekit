...
function fppHomekitQRCode() {
    ...
    // NOTE: Unverified/custom HomeKit accessories may not support QR code pairing and may only work with manual setup code entry
    $qrData = null;
    if (file_exists($infoFile)) {
        $infoData = @file_get_contents($infoFile);
        if ($infoData) {
            $info = @json_decode($infoData, true);
            if ($info && isset($info['qr_data']) && !empty($info['qr_data'])) {
                $qrData = $info['qr_data'];
            }
        }
    }
    ...
    if (!$qrData) {
        ...
        // IMPORTANT: Setup ID must be hexadecimal (0-9, A-F only) for X-HM format
        // If setup_id contains non-hex characters (like 'HOME'), convert to hex or use default
        $setupIDHex = strtoupper($setupID);
        // Remove any non-hexadecimal characters (only allow 0-9, A-F)
        $setupIDHex = preg_replace('/[^0-9A-F]/', '', $setupIDHex);
        
        // If after filtering we have no valid hex characters, use a default hex value
        if (empty($setupIDHex)) {
            $setupIDHex = '00000000'; // Default hex setup ID
        } else {
            // Take first 8 hex characters, pad with zeros if needed
